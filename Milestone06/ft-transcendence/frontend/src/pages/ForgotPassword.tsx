...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(translateApiMessage(err instanceof Error ? err.message : 'Failed to send reset email', t));
    } finally {
      setLoading(false);
    }
  };
  ...
  const cardContent = success ? (
    <>
      <h1 className="text-2xl font-bold text-white mb-6">{t('forgotPassword.title')}</h1>
      <p className="text-green-400 mb-4">{t('forgotPassword.successMessage')}</p>
      <Link
        to="/login"
    ...
    <>
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-400 font-medium">{t('forgotPassword.title')}</p>
      </div>
      <p className="text-gray-400 text-sm">{t('forgotPassword.description')}</p>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="text-gray-400 text-xs ml-1">{t('forgotPassword.email')}</label>
          <input
            type="email"
            value={email}
